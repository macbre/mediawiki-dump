...
def test_dump_get_url():
    ...
    assert WikipediaDump('fo', full_history=False).get_url() == \
        'https://dumps.wikimedia.org/fowiki/latest/fowiki-latest-pages-meta-current.xml.bz2'

    assert WikipediaDump('fo', full_history=True).get_url() == \
    ...
    assert WikiaDump('plpoznan', full_history=True).get_url() == \
